...
export default function InspectSubjectScreen() {
  ...
  const handleInspect = async (offer) => {
    if (connectivity.isOffline) {
      topAlert.show('Necesitas conexi�n a internet para ver los detalles.', 'info');
      return;
    }
    setRowLoading((prev) => ({ ...prev, [offer.id]: true }));
    try {
      if (!ready || !user) {
        topAlert.show('Debes iniciar sesi�n para reservar una tutor�a', 'info');
        return;
      }
    ...
    >
      ...
      {(connectivity.isOffline || fromCache) && (
        <View style={styles.offlineBanner}>
          <Text style={styles.offlineBannerText}>Datos guardados para uso sin conexi�n</Text>
        </View>
      )}
      ...
      {empty && (
        <View style={styles.row}>
          <Text style={styles.rowTitle}>No hay clases disponibles todav�a.</Text>
        </View>
      )}
          ...
          <View key={it.id} style={styles.row}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.rowSub}>
                Cupos: {max === 0 ? 'Sin l�mite' : `${enrolled}/${max}`}
              </Text>
              {firstSlot && (
